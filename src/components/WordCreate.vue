...
const { checkValidate } = storeToRefs(store);

const emit = defineEmits(["wordRemove", "wordUpdate"]);
...
const wordToArray = reactive(wordData.word.split(""));

const isKeyDownEvent = ref(false);

const isEmptyTheme = computed(() => {
  return checkValidate.value && wordData.theme.trim() === "";
});

const isEmptyWord = computed(() => {
...
watch(wordToArray, (newValue, oldValue) => {
